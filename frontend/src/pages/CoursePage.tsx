  ...
  if (!course) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        <p className="text-slate-500 dark:text-slate-400">Couldn’t find {code}.</p>
        <button
          onClick={onBack}
          className="mt-4 text-slate-900 dark:text-slate-100 underline"
        >
          Go back
        </button>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex items-baseline gap-3 flex-wrap">
        ...
        <span className="text-xl text-slate-700 dark:text-slate-300">{course.name}</span>
      </div>
      ...
      <details className="mt-8 group">
        <summary className="cursor-pointer list-none text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 flex items-center gap-2">
          <span className="text-slate-400 dark:text-slate-500 transition-transform group-open:rotate-90">
            ▸
          </span>
        ...
        <div className="mt-3">
        {history.length === 0 ? (
          <p className="text-slate-400 dark:text-slate-500">No recorded offerings yet.</p>
        ) : (
          <div className="space-y-4">
            {history.map(([label, offerings]) => (
              ...
              >
                <div className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                  {label}
                ...
                <ul className="divide-y divide-slate-100 dark:divide-slate-800">
                  {offerings.map((offering) => (
                    ...
                    >
                      <span className="text-slate-400 dark:text-slate-500 w-12 shrink-0">
