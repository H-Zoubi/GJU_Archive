...
export default function PdfViewer({ url }: { url: string }) {
  ...
  return (
    <div className="flex h-full flex-col bg-slate-100 dark:bg-slate-950">
      <div className="flex flex-1 justify-center overflow-auto">
        ...
        >
          <Document
            ...
            loading={<p className="py-16 text-sm text-slate-400 dark:text-slate-500">Loading preview…</p>}
          >
            <Page
      ...
      {!!numPages && numPages > 1 && (
        <div className="flex shrink-0 items-center gap-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-2">
          <button
            onClick={() => setPageNumber((n) => Math.max(1, n - 1))}
            disabled={pageNumber <= 1}
            className="rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-1 text-sm text-slate-700 dark:text-slate-300 disabled:opacity-30"
          >
            Previous
          </button>
